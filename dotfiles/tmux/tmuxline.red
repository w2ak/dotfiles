# This tmux statusbar config was created by tmuxline.vim
# on Thu, 19 Oct 2017

set -g status-justify "left"
set -g status "on"
set -g status-style "none"
set -g message-command-style bg="colour235"
set -g status-left-length "100"
set -g pane-active-border-style fg="colour88"
set -g status-bg "colour233"
set -g message-command-style fg="colour231"
set -g pane-border-style fg="colour235"
set -g message-style bg="colour235"
set -g status-left-style "none"
set -g status-right-style "none"
set -g status-right-length "100"
set -g message-style fg="colour231"
setw -g window-status-style fg="colour231"
setw -g window-status-style "none"
setw -g window-status-activity-style bg="colour233"
setw -g window-status-activity-style "none"
setw -g window-status-activity-style fg="colour88"
setw -g window-status-separator ""
setw -g window-status-style bg="colour233"
set -g status-left "#[fg=colour231,bg=colour88] #S #[fg=colour88,bg=colour233,nobold,nounderscore,noitalics]"
set -g status-right "#[fg=colour235,bg=colour233,nobold,nounderscore,noitalics]#[fg=colour231,bg=colour235] %Y-%m-%d  %H:%M #[fg=colour88,bg=colour235,nobold,nounderscore,noitalics]#[fg=colour231,bg=colour88] #h "
setw -g window-status-format "#[fg=colour231,bg=colour233] #I #[fg=colour231,bg=colour233] #W "
setw -g window-status-current-format "#[fg=colour233,bg=colour235,nobold,nounderscore,noitalics]#[fg=colour231,bg=colour235] #I #[fg=colour231,bg=colour235] #W #[fg=colour235,bg=colour233,nobold,nounderscore,noitalics]"
